...
class DatePartitioner extends Partitioner<IntWritable, Text> implements Configuration{
	
	private static final String ACCESS_DATE_MONTH = "access.date.month";
	
	private Configuration conf = null;
	private int accessDateMonth = 0;
	
	public int getPartition(IntWritable key, Text value, int numPartitions) {
		return key.get() - accessDateMonth;
	}
	...
	public void setConf(Configuration conf) {
		this.conf = conf;
		accessDateMonth = conf.getInt(ACCESS_DATE_MONTH, 0);
	}
	
	public static void setAcessDate (Job job, int accessDateMonth) {
		job.getConfiguration().setInt(ACCESS_DATE_MONTH,accessDateMonth);
	}
